...
def logistic_distance_and_locxy(train_df, test_df, weight=10.0):
    ...
    fit_y = np.expand_dims(train_df["shot_made_flag"].values, 1)

    weights = np.ones(len(fit_y))
    ind, = np.where(train_df["shot_distance"] > 40)
    weights[ind] = weight

    fit_x = train_df[["shot_distance", "loc_x", "loc_y"]].values
    predict_x = test_df[["shot_distance", "loc_x", "loc_y"]].values
...
if __name__ == "__main__":
    ...
    # predict_y = logistic_distance_and_locxy(df_trn, df_tst)
    # makeSubmission(predict_y, savename="submission_logistic.txt")
